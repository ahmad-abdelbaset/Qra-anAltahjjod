<script>
    ...
    const dic =[
        {sura:'سورة الفاتحة',id:'1'},
        {sura:'سورة البقرة',id:'2'}
    ]
...
function goToPlaylist(id) {
    // Construct the URL with the playlist's ID and navigate to it
  ...
  <div class="button-container">
    ...
    <button class="playlist-button" on:click={() => goToPlaylist(playlist.id)}>
      {playlist.id} - {playlist.title}
    </button>
  {/each}

      <!--
    {#each dic as item, index}
      <button class="playlist-button"> {item.id} - {item.sura}</button>
    {/each}
  
    {#each dic as item, index}
    <button class="playlist-button" onclick="goToPlaylist({item.id})">
      {item.id} - {item.sura}
    </button>
  {/each}-->

  <a href="/playlist/1">one</a>
...
<style>
    ...
    ul { /* Add this class for right alignment */
      display: flex;
  ...
  /* Style for the playlist buttons */
  .playlist-button {
    background-color: #fff; /* White background color */
    color: #444; /* Text color (initial color) */
    border: 1px solid #ddd;
    padding: 12px 20px;
    margin: 0px 0; /* Add margin to separate buttons vertically */
    cursor: pointer;
    font-size: 16px;
  ...
  /* Change text color to gray when hovering */
  .playlist-button:hover {
    background-color: rgb(230, 222, 222); /* Gray text color on hover */
  }
